...
# Build and Output Throughput Plot ------------------------
  output$Both__3_ <- renderPlotly({
    plot = dfB1() %>% ggplot() + 
      geom_histogram(mapping = aes(x=distancefromhome, colour = department, fill = department), binwidth = 10) +
      theme_bw() +
      ...
      ggtitle(paste(Departments(), input$title__3_)) +
    ...
    if( ! is.na(input$plotWidth__3_) & ! is.na(input$plotHeight__3_))
     ggplotly(plot, tooltip = c("distancefromhome"), session="knitr", width = input$plotWidth__3_, height = input$plotHeight__3_)
    else
      ggplotly(plot, tooltip = c("distancefromhome"), session="knitr") 
  })
# ---------------------------------------------------------
